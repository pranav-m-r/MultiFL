...
# Global store for client-specific private head state_dicts
CLIENT_PRIVATE_HEAD_STORE: Dict[str, OrderedDict[str, torch.Tensor]] = {}

class MultimodalFlowerClient(NumPyClient):
    def __init__(self, model, trainloader, valloader, local_epochs, partition_id: int):
        self.model = model
        ...
        self.private_head_state_dict: Optional[OrderedDict[str, torch.Tensor]] = CLIENT_PRIVATE_HEAD_STORE.get(self.client_cid)
    ...
    def fit(self, parameters: List[np.ndarray], config: dict) -> Tuple[List[np.ndarray], int, Dict]:
        ...
        self.client_cid = config.get("client_cid")
        ...
        try:
            ...
            # Overwrite head_module with private head if it exists
            if self.private_head_state_dict:
                # Ensure tensors are on the correct device before loading
                head_state_to_load = OrderedDict()
                for k, v_tensor in self.private_head_state_dict.items():
                    head_state_to_load[k] = v_tensor.to(self.device)
                self.model.head_module.load_state_dict(head_state_to_load)
                # print(f"Client CID {self.client_cid} : Restored private head for training.")
            ...
            # Save the updated private head_module's state_dict (to instance and global store)
            ...
            self.private_head_state_dict = current_head_state_cpu
            CLIENT_PRIVATE_HEAD_STORE[self.client_cid] = self.private_head_state_dict
            # print(f"Client CID {self.client_cid} : Saved private head for training.")
    ...
    def evaluate(self, parameters: List[np.ndarray], config: dict) -> Tuple[float, int, Dict]:
        ...
        try:
            ...
            # Overwrite head_module with private head if it exists for evaluation
            if self.private_head_state_dict:
                head_state_to_load = OrderedDict()
                for k, v_tensor in self.private_head_state_dict.items():
                    head_state_to_load[k] = v_tensor.to(self.device)
                self.model.head_module.load_state_dict(head_state_to_load)
                # print(f"Client CID {self.client_cid} : Restored private head for evaluation.")
